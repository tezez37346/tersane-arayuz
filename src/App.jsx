...
export default function App() {
  ...
  const runOptimization = async () => {
    ...
    try {
      const response = await fetch(
        `https://tersane-arayuz.onrender.com/solve?block_count=${blockCount}&priority=${encodeURIComponent(priorityType)}`
      );
  ...
  const filteredAssignments = useMemo(() => {
    ...
    return result.assignments.filter(
      (a) => String(a.block) === selectedBlock
    );
    ...
    <div style={page}>
      ...
      <main style={main}>
        <div style={topBar}>
          <div>
            <h1 style={title}>
              TERSANE İŞGÜCÜ KARAR DESTEK SİSTEMİ
            </h1>

            <p style={subtitle}>
              En uygun işçi ataması için çok kriterli optimizasyon
            </p>
          </div>
          ...
          <>
            ...
            {!result || !dashboard ? (
              <div style={emptyDashboard}>
                Analiz çalıştırıldığında uygun işçi önerisi,
                karşılaştırma tablosu ve grafikler otomatik olarak
                oluşturulacaktır.
              </div>
            ) : (
              <>
                <div style={metricGrid}>
                  <Metric
                    icon="👥"
                    title="Uygun İşçi Sayısı"
                    value={`${dashboard.eligible_workers} / ${dashboard.total_workers}`}
                    note="Sertifika uygun işçi"
                  />

                  <Metric
                    icon="⏱️"
                    title="En İyi İşlem Süresi"
                    value={`${dashboard.best_duration} saat`}
                    note="Tahmini süre"
                  />

                  <Metric
                    icon="💛"
                    title="En Düşük Ergonomik Risk"
                    value={`${dashboard.min_ert} / 100`}
                    note="ERT değeri"
                  />

                  <Metric
                    icon="⭐"
                    title="En Yüksek Uygunluk Skoru"
                    value={dashboard.best_score}
                    note="0-1 arası normalize skor"
                  />
                ...
                <div style={analysisGrid}>
                  <div style={recommendCard}>
                    ...
                    <div style={successNote}>
                      ✅ Bu işçi seçildiğinde modelin toplam
                      amacına en iyi katkı sağlanmaktadır.
                    </div>
                  </div>
        ...
        {activePage === "İş Adımları" && (
          <section style={panel}>
            ...
            <p>
              CNC Ön İmalat → CNC Panel → Ön İmalat / Panel →
              Blok İmalat → Ön Donatım → Erection → Boya
            </p>
          </section>
        )}
        ...
        {activePage === "İşçiler" && (
          <section style={panel}>
            ...
            <p>
              Modelde 51 işçi ve 15 sertifika türü dikkate
              alınmaktadır.
            </p>
          </section>
        )}
            ...
            {!result ? (
              ...
              <>
                ...
                >
                  ...
                  {[
                    ...new Set(
                      result.assignments.map((a) => a.block)
                    ),
                  ].map((b) => (
        ...
        {activePage === "Raporlar" && (
          <section style={panel}>
            ...
            {!result ? (
              ...
              <p>
                Model {result.block_count} blok için
                çözülmüş, Temin Süresi değeri {result.cmax}
                ve amaç fonksiyonu değeri{" "}
                {result.objective} olarak elde edilmiştir.
              </p>
            )}
          </section>
        )}
        ...
        {activePage === "Ayarlar" && (
          <section style={panel}>
            ...
            <p>
              Bu bölümde ileride model ağırlıkları ve analiz
...
function Metric({ icon, title, value, note, green }) {
  return (
    <div style={metricCard}>
      ...
      <h2 style={{ color: green ? "#16a34a" : "#0f172a" }}>
        {value}
      </h2>
